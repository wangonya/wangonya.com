...
[[https://wangonya.com/blog/click-commands-options/][options]].
If you're familiar with functional programming, then you're familiar
with arguments. The concept is the same in Click.
...
We specify that =hello()= should expect an argument =person= to be
passed in when it's called, and add a default value so that it's ok to
call the command without passing in the argument.
