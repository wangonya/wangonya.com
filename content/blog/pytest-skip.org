...
others for some reason. Maybe the database guy isn't done setting up and
that particular test requires a database connection. Instead of having
to wait, you can just write the test and instruct pytest to skip it,
giving the appropriate reason so it doesn't look like you just skipped a
failing test to keep your test suite green.
...
Notice the little =s=. It shows the test that was skipped. Pytest also
tells us that =1 passed, 1 skipped=. If you need a more verbose output,
you can use the =-rs= flag =pytest skip.py -rs=:
...
[[https://docs.pytest.org/en/latest/skipping.html][docs]].
